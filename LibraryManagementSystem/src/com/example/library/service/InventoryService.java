...
public class InventoryService {
    ...
    public void updateBook(Book b) { bookRepo.update(b); }

    public Optional<Book> findBookByIsbn(String isbn) { return bookRepo.findByIsbn(isbn); }
    public List<Book> findByTitle(String title) { return bookRepo.findByTitle(title); }
    ...
    public List<Book> availableBooks() {
        ...
        return bookRepo.findAll().stream()
                .filter(b -> !borrowed.contains(b.getIsbn()))
    ...
    public boolean checkout(String isbn, String patronId, int days) {
        ...
        if (maybeBook.isEmpty() || maybePatron.isEmpty()) return false;
    ...
    public void reserve(String isbn, String patronId) {
        Optional<Book> maybeBook = bookRepo.findByIsbn(isbn);
        Optional<Patron> maybePatron = patronRepo.findById(patronId);
        if (maybeBook.isEmpty() || maybePatron.isEmpty()) return;
    ...
    public List<Book> recommendFor(String patronId, int limit) {
        Optional<Patron> maybePatron = patronRepo.findById(patronId);
        if (maybePatron.isEmpty() || recommendationStrategy == null) return Collections.emptyList();
        List<Book> available = availableBooks();
        List<Book> recs = recommendationStrategy.recommend(maybePatron.get(), available);
        if (recs.size() > limit) return recs.subList(0, limit);
        return recs;
    }

    public boolean isBorrowed(String isbn) { return activeLoans.containsKey(isbn); }
}
